/* Create a temp table with 1 players game by game hitting data only */
DROP TABLE IF EXISTS tmp_batting_data;
...
WHERE BAT_ID = 'ortid001' AND AB_FL = 'T'
...
WHERE BAT_ID = 'ortid001' AND (EVENT_CD > 13 AND EVENT_CD < 17)
GROUP BY BAT_ID, GAME_ID, YEAR_ID);
...
WHERE BAT_ID = 'ortid001' AND BAT_EVENT_FL = 'T'
GROUP BY BAT_ID, GAME_ID, YEAR_ID);

# SELECT * FROM tbl_plateapps;

CREATE TABLE tmp_batting_data (
...
INSERT INTO tmp_batting_data (BAT_ID, GAME_ID, YEAR_ID, AT_BATS, SLUGGING, AVERAGE, HITS, OBP, PLATE_APPS, WALKS)
...
ON w.GAME_ID = ab.GAME_ID
INNER JOIN tbl_plateapps pa
ON pa.GAME_ID = ab.GAME_ID;
